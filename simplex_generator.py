...
def simplex_params(bot_dim: int, input_working_device: torch.device) -> Simplex:
    ...
    # Move the operation to torch so that it doesn't overload the cpu
    sigma_real_tensor = torch.tensor(sigma_real, device=input_working_device, dtype=torch.float32)
    sigma_det_rsqrt = torch.rsqrt(2 * np.pi * torch.linalg.det(sigma_real_tensor))  # GPU version of determinant and inverse sqrt

    # Use torch.linalg.inv for GPU support
    sigma_inv = torch.linalg.inv(sigma_real_tensor)
